...
export async function antilink(client, message) {
    ...
    try {
        ...
        if (!action) {
            const usage = `ðŸ”’ *Digital Crew 243 - Antilink*\n\n.antilink on\n.antilink off\n.antilink set delete | kick | warn\n.antilink status`
        ...
        switch (action) {
            ...
            case 'set':
                if (args.length < 2) {
                    return await client.sendMessage(groupId, { 
                        text: 'âŒ Usage: .antilink set delete | kick | warn' 
                    })
                }
                const setAction = args[1].toLowerCase()
                if (!['delete', 'kick', 'warn'].includes(setAction)) {
                    return await client.sendMessage(groupId, { 
                        text: 'âŒ Actions: delete, kick, warn' 
                    })
                }
            ...
            case 'status':
                ...
                await client.sendMessage(groupId, { 
                    text: `ðŸ“Š *Statut*\n\nActivÃ©: ${status?.enabled ? 'âœ…' : 'âŒ'}\nAction: ${status?.action || 'Aucune'}` 
                })
                break

            default:
                await client.sendMessage(groupId, { 
                    text: 'âŒ Usage: .antilink on/off/set/status' 
                })
        }
    } catch (error) {
...
export async function linkDetection(client, message) {
    console.log('ðŸ” LINK DETECTION CALLED')
    
    const groupId = message.key.remoteJid
    if (!groupId.includes('@g.us')) {
        console.log('ðŸŸ¡ Not a group')
        return
    }
    
    const setting = antilinkSettings[groupId]
    if (!setting?.enabled) {
        console.log('ðŸŸ¡ Antilink disabled for group')
        return
    }
    ...
    if (!hasLink) {
        console.log('ðŸŸ¡ No link found')
        return
    }
    
    console.log('ðŸŸ¢ Link detected!')
    ...
    try {
        ...
        if (sender?.admin) {
            console.log('ðŸŸ¡ Sender is admin, skipping')
            return
        }
        
        if (!bot?.admin) {
            console.log('ðŸŸ¡ Bot not admin, skipping')
            return
        }
        
        console.log('ðŸŸ¢ Taking action:', setting.action)
        ...
        if (setting.action === 'delete' || setting.action === 'kick' || setting.action === 'warn') {
            try {
                ...
                console.log('âŒ Delete failed:', deleteError.message)
            }
        }
...
export async function resetwarns(client, message) {
    ...
    if (message.message?.extendedTextMessage?.contextInfo?.quotedMessage) {
        ...
        return await client.sendMessage(groupId, {
            text: `ðŸ“Š *Warns:* ${count} utilisateur(s)\n\nUsage: .resetwarns @user`
        })
    }
